...
//       alert(storeToJson(store.QDrug)); return;
        ...
        //debugger;
        var flag = 0;
        if (document.getElementById('btnCanSave').innerText == "取消") {
            flag = 1;
        } else {
            flag = 2;
        }
        ...
        App.direct.SaveJsonInfo(storeToJson(store.main), storeToJson(store.mixing), weightStoreToJson(store.weight), weightStoreToJson(store.advance),
            ...
            storeToJson(store.mix4), storeToJson(store.mix5), storeToJson(store.mix6), storeToJson(store.QDrug), storeToJson(store.MILL), storeToJson(store.QDrug2), storeToJson(store.CMILL), flag,{
...
var setEditable = function () {
    ...
    try {
        App.Panel1.setActiveTab(App.pnlMain);
