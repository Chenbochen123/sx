...
var Save2 = function () {
    var record = App.DetailPanel.getRecord();
    if ((record) && (record.data.ObjID > 0)) {
        Ext.Msg.confirm("提示", '您确定需要修改此工艺配方信息？', function (btn) { doSave2(btn); });
    }
}

var doSave2 = function (btn) {
    if (btn != "yes") {
        return;
    }
    var after = function () {
    ...
    try {
        var record = App.DetailPanel.getRecord();
        var recipe = record.data.ObjID;
        var cheshu = App.txtcheshu.getValue();
        App.direct.SaveRecipeInfo2(recipe, cheshu, {
            success: function (result) {
                after();
                if (result == "") {
                    Ext.Msg.alert('成功', "车数修改成功！", function (btn) { gridPanelRefresh() });
                } else {
                    Ext.Msg.alert('失败', result);
                }
            },
