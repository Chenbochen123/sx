            ...
            App.direct.chartMainBindData({
                success: function (result) {
                    ...
                    var cpkData = [
                        { col1: '总样本数', col2: data.length, col3: '组数', col4: 10, col5: '子组大小', col6: 5 },
                        { col1: '平均值', col2: std.mean.toFixed(2), col3: '最大值', col4: std.max, col5: '最小值', col6: std.min },
                        { col1: '标准值', col2: std.value.toFixed(2), col3: '上限值', col4: arr[2], col5: '下限值', col6: arr[3] },
                        { col1: '', col2: '', col3: '', col4: '', col5: '', col6: '' },
                        { col1: '标准差', col2: std.stdev.toFixed(2), col3: '公差T', col4: std.T.toFixed(2), col5: '中心值U', col6: std.value.toFixed(2) },
                        { col1: 'Ca', col2: std.Ca.toFixed(2), col3: 'Cp', col4: std.Cp.toFixed(2), col5: 'Cpk', col6: stdNew.Cpk.toFixed(2) },
                        { col1: 'Ppk', col2: std.Ppk.toFixed(2), col3: '', col4: '', col5: '', col6: '' }
                    ];
...
<body>
    ...
    <script type="text/javascript">
        ...
        var DrawLine = function (data, std) {
            ...
            var yArrNew = [];
            for (var m = 1; m < data.length + 1; m = m + 1) {
                var arr = [];
                arr.push(m);
                arr.push(data[m-1]);
                yArrNew.push(arr);
            }
            //console.log("yArrNew:" + yArrNew);
            ...
            optionLine.series[0].data = yArrNew;
            optionLine.series[0].name = "";
            optionLine.yAxis[0].max = std.max >= std.up ? (std.max * 1.02).toFixed(2) : (std.up * 1.02).toFixed(2);
            optionLine.yAxis[0].min = std.min <= std.down ? (std.min * 0.98).toFixed(2) : (std.down * 0.98).toFixed(2);
            //optionLine.xAxis[0].max = data.length;
            //optionLine.xAxis[0].min = 1;
