...
namespace Mesnac.Data.Implements
{
    ...
    public class PpmRubberStoreoutService : BaseService<PpmRubberStoreout>, IPpmRubberStoreoutService
    {
		#region 构造方法

        public PpmRubberStoreoutService() : base(){ }

        public PpmRubberStoreoutService(string connectStringKey) : base(connectStringKey){ }

        public PpmRubberStoreoutService(NBear.Data.Gateway way) : base(way){ }
        ...
        public class QueryParams
        {
            ...
            public PageResult<PpmRubberStoreout> pageParams { get; set; }
        }

        public PageResult<PpmRubberStoreout> GetTablePageDataBySql(QueryParams queryParams)
        {
            PageResult<PpmRubberStoreout> pageParams = queryParams.pageParams;
        ...
        {
            int rows = this.GetBySql("select BillNo from PpmRubberStoreout where DeleteFlag = '0' and SUBSTRING(BillNo, 3, 6) = SUBSTRING(CONVERT(varchar(10),GETDATE(), 112), 3, 6)").ToDataSet().Tables[0].Rows.Count;
            if (rows > 0)
                return this.GetBySql("select 'CK' + SUBSTRING(CONVERT(varchar(10),GETDATE(), 112), 3, 6) + RIGHT('000' + CONVERT(VARCHAR, MAX(CONVERT(int, RIGHT(BillNo, 3))) + 1), 3) from PpmRubberStoreout where DeleteFlag = '0' and SUBSTRING(BillNo, 3, 6) = SUBSTRING(CONVERT(varchar(10),GETDATE(), 112), 3, 6)").ToDataSet().Tables[0].Rows[0][0].ToString();
            else
        ...
        public bool UpdateLockedFlag(string StrBillNo, string ChkPerson)
        {
            ...
            {
                sql = @"update PpmRubberStoreout set LockedFlag = '1', ChkPerson = '" + ChkPerson + "', ChkResultFlag = '1', ChkDate = GETDATE() where BillNo in (" + StrBillNo + ")";
        ...
        public bool CancelLocked(string StrBillNo, string ChkPerson)
        {
            ...
            if (!string.IsNullOrEmpty(StrBillNo))
            {
                sql = @"update PpmRubberStoreout set LockedFlag = '0', ChkPerson = '" + ChkPerson + "', ChkResultFlag = '0', ChkDate = null where BillNo in (" + StrBillNo + ")";
                this.GetBySql(sql).ExecuteNonQuery();
                return true;
            }
            return false;
        ...
        {
            DataSet ds = this.GetBySql(@"select A.BillNo, A.StorageID, B.Barcode, B.MaterCode from PpmRubberStoreout A
                                        left join PpmRubberStoreoutDetail B on A.BillNo = B.BillNo
