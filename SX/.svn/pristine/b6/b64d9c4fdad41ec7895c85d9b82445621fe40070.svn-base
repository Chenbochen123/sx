...
public partial class Manager_System_ShowRoleAction_ShowUserActionEqual : Mesnac.Web.UI.Page
{
    #region 权限定义
    ...
    {
        string user = hiddenUser2Code.Text;
        if (string.IsNullOrWhiteSpace(user))
        {
            return new { data = new EntityArrayList<BasUser>(), total = 0 };
        }
        EntityArrayList<SysUserActionEqual> userList = sysUserActionEqualManager.GetListByWhere(SysUserActionEqual._.User2Code == user);
        EntityArrayList<BasUser> lst = new EntityArrayList<BasUser>();
        foreach (SysUserActionEqual m in userList)
        {
            lst.Add(basUserManager.GetListByWhere(BasUser._.WorkBarcode == m.User1Code)[0]);
        }
        return new { data = lst, total = lst.Count };
    }
