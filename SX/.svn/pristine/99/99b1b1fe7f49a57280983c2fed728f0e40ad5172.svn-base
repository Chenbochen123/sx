<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MaterialStorageAdjustInsert.aspx.cs" Inherits="Manager_ShopStorage_MaterialStorageAdjustInsert" %>
            ...
            App.direct.btnSave_Click({
                success: function (result) {
                    if (result == "false")
                        ...
                        Ext.Msg.alert('操作', "您选择的源库房物料小于您设置的退库数量，请检查！");
                    else if (result == "false2")
                        Ext.Msg.alert('操作', "明细表中的库位和库房对应的库位不对应，请检查调整！");
                    else {
                        var tabid = "id=186";
...
//                            parent.refresh("");
        ...
        var Manager_BasicInfo_CommonPage_QueryBasStorage_Request = function (record) {//库房信息返回值处理
            if (!App.winAdd.hidden) {
                ...
                App.txtTargetStorage1.setValue(record.data.StorageName);
                App.hiddenTargetStorageID.setValue(record.data.StorageID);
            }
        }
        var Manager_BasicInfo_CommonPage_QueryBasStoragePlace_Request = function (record) {//库位信息返回值处理
            if (!App.winAdd.hidden) {
                ...
                App.txtTargetStoragePlaceName2.setValue(record.data.StoragePlaceName);
                App.hiddenTargetStoragePlaceID.setValue(record.data.StoragePlaceID);
            }
            else {
        ...
        var Manager_BasicInfo_CommonPage_QueryDepartment_Request = function (record) {//部门信息返回值处理
            App.txtDeptCode1.setValue(record.data.DepName);
        ...
        var AddStoragePlace = function () {//库位添加
            var url = "../BasicInfo/CommonPage/QueryBasStoragePlace.aspx?StorageID=" + App.hiddenTargetStorageID.getValue();
        ...
        var AddDept = function (field, trigger, index) {//部门添加
            var url = "../BasicInfo/CommonPage/QueryDepartment.aspx?DeptLevel=2";
            var html = "<iframe src='" + url + "' width=100% height=100% scrolling=no  frameborder=0></iframe>";
            if (App.Manager_BasicInfo_CommonPage_QueryDepartment_Window.getBody()) {
                App.Manager_BasicInfo_CommonPage_QueryDepartment_Window.getBody().update(html);
            } else {
                App.Manager_BasicInfo_CommonPage_QueryDepartment_Window.html = html;
            }
        ...
        Ext.create("Ext.window.Window", {//部门带窗体
...
//        }
        ...
        var edit = function (editor, e) {
            if (e.value > e.record.data.RealWeight) {
                Ext.Msg.alert("提示", "您设置的重量超出当前库存重量，请重新设置！");
                e.record.reject();
            }
        };
        ...
        var SetFocusAdjustNum2 = function () {
            App.hiddenFocus.setValue("txtAdjustNum2");
        };

        var SetFocusAdjustWeight2 = function () {
            App.hiddenFocus.setValue("txtAdjustWeight2");
        };

        var shiftClassIDChange = function (value) {
            if (value == "1") return "甲";
            else if (value == "2") return "乙";
            else if (value == "3") return "丙";
            //return Ext.String.format(value ? "已作废" : "否");
        };

        var shiftIDChange = function (value) {
            if (value == "1") return "早班";
            else if (value == "2") return "中班";
            else if (value == "3") return "夜班";
        };
    </script>
</head>
<body>
    ...
    <ext:Viewport ID="vpAdjustBill" runat="server" Layout="border">
        <Items>
            <ext:Panel ID="pnAdjustBill" runat="server" Region="North" Height="90">
                <TopBar>
                    <ext:Toolbar runat="server" ID="tbAdjustBill">
                        <Items>
                            <ext:ToolbarSeparator ID="tsBegin" />
                            <ext:Button runat="server" Icon="TableEdit" Text="生成退库单" ID="GenStorageOut" Disabled="true">
                                <Listeners>
                    ...
                    <ext:FormPanel ID="container_top" runat="server" Layout="ColumnLayout" AutoHeight="true">
                        <Items>
                            <ext:Container ID="container4" runat="server" Layout="FormLayout" ColumnWidth=".33"
                                Padding="5">
                                <Items>
                                    <ext:TextField ID="txtBillNo1" runat="server" FieldLabel="退库单号" Text="(自动生成)" LabelAlign="Right" ReadOnly="true" />
                                    <ext:DateField ID="txtAdjustDate1" runat="server" FieldLabel="退库日期" LabelAlign="Right" IndicatorText="*" IndicatorCls="red-text" AllowBlank="false" />
                                </Items>
                            </ext:Container>
                            <ext:Container ID="container5" runat="server" Layout="FormLayout" ColumnWidth=".33"
                                Padding="5">
                                <Items>
                                    <%--<ext:TriggerField ID="txtDeptCode1" runat="server" FieldLabel="发往部门" LabelAlign="Left" IndicatorText="*" IndicatorCls="red-text" AllowBlank="false"
                                        Editable="false">
                                        <Triggers>
                                        ...
                                        <Listeners>
                                            <TriggerClick Fn="AddDept" />
                                        </Listeners>
                                    </ext:TriggerField>--%>
                                    <ext:TriggerField ID="txtTargetStorage1" runat="server" FieldLabel="发往库房" LabelAlign="Right" Editable="false" IndicatorText="*" IndicatorCls="red-text"  AllowBlank="false">
                                        <Triggers>
                                            <ext:FieldTrigger Icon="Search" />
                                        </Triggers>
                                        <RemoteValidation OnValidation="CheckField" />
                                    ...
                                    <ext:TextField ID="txtMakerPerson1" runat="server" FieldLabel="制单人" LabelAlign="Right" ReadOnly="true" />
                                </Items>
                            </ext:Container>
                            <ext:Container ID="container6" runat="server" Layout="FormLayout" ColumnWidth=".33"
                                ...
                                <Items>
                                    ...
                                    <ext:TextField ID="txtSourceStorage1" runat="server" LabelAlign="Right" FieldLabel="源库房" ReadOnly="true" />
                                    <ext:TextField ID="txtRemark1" runat="server"  LabelAlign="Right" FieldLabel="备注" />
                                </Items>
                            </ext:Container>
                        </Items>
                        <Listeners>
                    ...
                    <ext:GridPanel ID="pnlTempList" runat="server">
                        <Store>
                            <ext:Store ID="store1" runat="server" PageSize="10">
                                ...
                                <Model>
                                    <ext:Model ID="model1" runat="server" IDProperty="Barcode,SourceStoragePlace,TargetStoragePlace">
                                        <Fields>
                                            <ext:ModelField Name="Barcode" />
                                            <ext:ModelField Name="ProductNo" />
                                            <ext:ModelField Name="SourceStoragePlace" />
                        ...
                        <ColumnModel ID="ColumnModel1" runat="server">
                            <Columns>
                                ...
                                <ext:Column ID="Column2" runat="server" Text="批次号" DataIndex="ProductNo" Flex="1" />
                                <ext:Column ID="Column3" runat="server" Text="源库位" DataIndex="SourceStoragePlaceName" Flex="1" />
                                <ext:Column ID="Column7" runat="server" Text="目的库位" DataIndex="TargetStoragePlaceName" Flex="1" />
                                ...
                                <ext:Column ID="Column5" runat="server" Text="退库数量" DataIndex="AdjustNum" Flex="1" />
                                <ext:Column ID="Column6" runat="server" Text="退库重量" DataIndex="AdjustWeight" Flex="1" />
                                <ext:Column ID="Column8" runat="server" Text="班组" DataIndex="ShiftClassID" Flex="1"><Renderer Fn="shiftClassIDChange" /></ext:Column>
                                <ext:Column ID="Column9" runat="server" Text="班次" DataIndex="ShiftID" Flex="1"><Renderer Fn="shiftIDChange" /></ext:Column>
            ...
            <ext:Window ID="winModifyDetail" runat="server" Icon="MonitorEdit" Closable="true" Title="修改退库明细信息"
                ...
                <Items>
                    <ext:FormPanel ID="FormPanel1" runat="server" BodyPadding="5">
                        ...
                        <Items>
                            ...
                            <ext:TextField ID="txtSourceStoragePlaceName2" runat="server" FieldLabel="源库位" LabelAlign="Left" ReadOnly="true" />                          
                            <ext:TriggerField ID="txtTargetStoragePlaceName2" runat="server" FieldLabel="目的库位" IndicatorText="*" IndicatorCls="red-text" AllowBlank="false" LabelAlign="Left" Editable="false">
                                <Triggers>
                                    <ext:FieldTrigger Icon="Search" />
                                </Triggers>
                                <Listeners>
                            ...
                            <ext:TextField ID="txtMaterialName2" runat="server" FieldLabel="物料名称" ReadOnly="true" LabelAlign="Left" />
                            <ext:DateField ID="txtProcDate2" runat="server" Format="yyyy-MM-dd" FieldLabel="生产日期" LabelAlign="Left" ReadOnly="true" />
                            <ext:TextField ID="txtAdjustNum2" runat="server" Vtype="integer" FieldLabel="当前数量" LabelAlign="Left" IndicatorText="*" IndicatorCls="red-text" AllowBlank="false" >
                                <%--<Listeners>
                                    <Focus Fn="SetFocusAdjustNum2" />
                                </Listeners>--%>
                            </ext:TextField>
                            <ext:TextField ID="txtPieceWeight2" runat="server" FieldLabel="单重" LabelAlign="Left" ReadOnly="true" />
                            <ext:TextField ID="txtAdjustWeight2" runat="server" Vtype="decimal" FieldLabel="实际重量" LabelAlign="Left" OnDirectChange="txtAdjustWeight2_Change" IndicatorText="*" IndicatorCls="red-text" AllowBlank="false">
                                <Listeners>
                                    <Focus Fn="SetFocusAdjustWeight2" />
                                </Listeners>
                            </ext:TextField>
                            <ext:TextField ID="txtRemark2" runat="server" FieldLabel="备注" LabelAlign="Left" />
                            <ext:ComboBox ID="cbxShiftClassID" runat="server" FieldLabel="班组" LabelAlign="Left">
                                <Items>
                                    <ext:ListItem Text="甲组" Value="1" AutoDataBind="true">
                                    </ext:ListItem>
                ...
                <Listeners>
                    <Show Handler="for(var i=0;i<#{vpAdjustBill}.items.length;i++){#{vpAdjustBill}.getComponent(i).disable(true);}" />
                    <Hide Handler="for(var i=0;i<#{vpAdjustBill}.items.length;i++){#{vpAdjustBill}.getComponent(i).enable(true);}" />
                </Listeners>
            </ext:Window>
            ...
            <ext:Window ID="winAdd" runat="server" Width="550" Height="550" Hidden="true" Title="库存查询"  Modal="false" Resizable="true" BodyStyle="background-color:#fff;"
                    ...
                    <ext:GridPanel ID="pnlList" runat="server" Height="370">
                                ...
                                <Model>
                                    <ext:Model ID="model" runat="server" IDProperty="Barcode,StorageID,StoragePlaceID">
                                        <Fields>
                                            ...
                                            <ext:ModelField Name="NewWeight" Type="Float" />
                        ...
                        <ColumnModel ID="colModel" runat="server">
                            <Columns>
                                ...
                                <ext:Column ID="NewWeight" runat="server" Text="设置重量" DataIndex="NewWeight" Flex="1">
                                    <Editor>
                                        <%--<ext:NumberField ID="NumberField1" runat="server" />--%>
                                        <ext:NumberField ID="NumberField" runat="server" MinValue="0" />
                                    </Editor>
                                </ext:Column>
                ...
                <Listeners>
                    <Show Handler="for(var i=0;i<#{vpAdjustBill}.items.length;i++){#{vpAdjustBill}.getComponent(i).disable(true);}" />
                    <Hide Handler="for(var i=0;i<#{vpAdjustBill}.items.length;i++){#{vpAdjustBill}.getComponent(i).enable(true);#{store}.reload();}" />
                </Listeners>
            </ext:Window>
            ...
            <ext:Hidden ID="hiddenSourceStorageID" runat="server"></ext:Hidden>
            <ext:Hidden ID="hiddenSourceStoragePlaceID" runat="server"></ext:Hidden>
            <ext:Hidden ID="hiddenTargetStorageID" runat="server"></ext:Hidden>
            <ext:Hidden ID="hiddenTargetStoragePlaceID" runat="server"></ext:Hidden>
