...
left join Eqm_Measure t1 on t.JL_code=t1.JL_code");
        ...
        {
            sb.AppendLine("AND JL_PlanDate>='" + DateBeginTime.SelectedDate.ToString("yyyy-MM-dd") + "'");
        }
        if (!string.IsNullOrEmpty(DateEndTime.SelectedDate.ToString()))
        {
            sb.AppendLine("AND JL_PlanDate<='" + DateEndTime.SelectedDate.ToString("yyyy-MM-dd") + "'");
        }
        #endregion
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (this.hideMode.Text == "Add")//添加
        {
            ...
            if (listAdd.Count > 0)
            {
                ...
                record.JL_planDate = txtJL_planDate.SelectedDate.ToString("yyyy-MM-dd");
                ...
                //if (string.IsNullOrEmpty(txtJL_RealDate.SelectedDate.ToString()))
                //{ record.JL_RealDate =null; }
                //else
                //{
                //    record.JL_RealDate = txtJL_RealDate.SelectedDate.ToShortDateString();
                //}
                if (txtJL_RealDate.SelectedDate != DateTime.MinValue)
                {
                    record.JL_RealDate = txtJL_RealDate.SelectedDate.ToString("yyyy-MM-dd");
                }
        ...
        else//修改
        {
            ...
            if (record != null)
            {
                ...
                EntityArrayList<Eqm_Measure> list1 = measureManager.GetListByWhere(Eqm_Measure._.JL_code == record.JL_code);
                if (list1.Count > 0)
                {
                    if (Convert.ToInt32(record.Plan_state) == 2)
                    {
                        if (list1[0].LastCheckDate < Convert.ToDateTime(record.JL_RealDate))
                        {
                            list1[0].LastCheckDate = Convert.ToDateTime(record.JL_RealDate);
                            list1[0].Mem_note = record.JL_RealDate;
                            list1[0].YX_Date = Convert.ToDateTime(record.JL_RealDate).AddMonths(Convert.ToInt32(list1[0].Check_stand));
                        }
                        else
                        {
                            X.Msg.Alert("提示", "校准日期小于最后检定日期，无法修改！").Show(); return;
                        }
                        measureManager.Update(list1[0]);
                    }
                }
    ...
    protected void btnSave_Click1(object sender, EventArgs e)
    {
        ...
        if (record != null)
        {
            ...
            if (list1.Count > 0)
            {
                ...
                {
                    X.Msg.Alert("提示", "校准日期小于最后检定日期，无法完成！").Show(); return;
                }
                measureManager.Update(list1[0]);
            }
